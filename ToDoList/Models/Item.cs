  ...
  public class Item
  {
    ...
    public static List<Item> GetAll()
    {
      List<Item> allItems = new List<Item> { };
        ...
        cmd.CommandText = @"SELECT * FROM items;"; // object to hold sql query
        MySqlDataReader rdr = cmd.ExecuteReader() as MySqlDataReader; //object to read result 
        while (rdr.Read())
        {   //translate row into object 
    ...
    {
      MySqlConnection conn = DB.Connection(); //DB is a class
      conn.Open();
      MySqlCommand cmd = conn.CreateCommand() as MySqlCommand;
      cmd.CommandText = @"DELETE FROM items;";
      cmd.ExecuteNonQuery(); //modify data
      conn.Close();
      if (conn != null)
      {
        conn.Dispose();
    ...
    public void Save()
    {
      ...
      description.Value = this.Description; //property
      cmd.Parameters.Add(description);    
      cmd.ExecuteNonQuery();
      Id = (int) cmd.LastInsertedId; //explicit conversion long into int
